...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    {
        pos_mat[5 * j + 0] = ntl->values[0 + j * ntl->dim] + 1;//x1
        pos_mat[5 * j + 2] = ntl->values[1 + j * ntl->dim] + 1;//x2
        pos_mat[5 * j + 1] = ntl->values[2 + j * ntl->dim] + 1;//y1
        pos_mat[5 * j + 3] = ntl->values[3 + j * ntl->dim] + 1;//y2
